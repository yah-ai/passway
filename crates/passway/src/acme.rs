...
/// @yah:parent(R853)
/// @yah:severity(medium)
/// @yah:next("DONE IN THIS TICKET'S OWN LATER PASS — retired 2026-09-06 (R870-T4), which went looking for the test and found it. As written: watch the interaction with the R779 issuance-failure backoff marker, since a config whose SAN set can never be satisfied (a name redundant with a wildcard, which LE rejects outright) would turn 'needs renewal' into a re-order loop. What shipped: both issuance paths go through one `issue_respecting_backoff`, so a third call site cannot reintroduce the gap, and `a_coverage_driven_renewal_still_respects_the_failure_backoff` (acme.rs:934) asserts an unexpired marker refuses before any order is attempted. Its sibling `refusing_under_backoff_does_not_escalate_it` pins the other half — a refusal is not a failure and must not double the backoff off orders that never happened.")
